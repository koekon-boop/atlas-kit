...
| [`weather`](../addons/weather/README.md) | `GET /api/weather`: current conditions + today's high/low for one place set in `.env`, read by the Jarvis tab | Nothing on disk, no key, no model call — one request to Open-Meteo per 15-min cache window, only while a dashboard asks |
| [`maps`](../addons/maps/README.md) | `POST /api/maps/route` (distance + ETA) and `POST /api/maps/nearby` (closest OSM points of interest), plus the same two as `mcpTools` (`maps_route`, `maps_nearby`) so the Jarvis chat can ask them directly. OSRM + Nominatim + Overpass — no navigation, no live traffic | Nothing on disk, no key, no model call — cached (5–60 min per endpoint), Nominatim rate-limited to 1 req/s in-process |
| [`whatsapp`](../addons/whatsapp/README.md) | A WhatsApp chat with the Atlas agent — one standing session per allowed number: Meta's Cloud API webhook (HMAC-verified, deduped, sender-allowlisted) → `POST /api/agents/spawn`/`prompt`/`queue`, and a bearer-gated `POST /api/whatsapp/send` the agent answers through. Text in and out; incoming voice notes are transcribed on the box through `addons/voice`'s `/api/voice/transcribe`; incoming pictures, videos and documents are saved on the box and handed to the agent as file paths (a video as a few ffmpeg stills plus its transcribed soundtrack); and a reply can go out as a read-aloud voice note (`voice: true` → `/api/voice/speak` → ffmpeg OGG/Opus → Meta media upload; any failure falls back to text) | One session id per sender, plus the pictures / videos / documents you send (kept 14 days in `<AGENT_LOCAL_DIR>/whatsapp-media/`, then swept; voice notes in and out are held in memory / a per-reply temp dir that is removed), no model call of its own; a voice reply costs one on-box TTS run per ~700 characters plus a short ffmpeg run — the chat session bills to your Claude subscription like any Atlas chat. Free Meta test number (5 verified recipients), 24-hour reply window; needs a public webhook path (Caddy block + a Cloudflare Access bypass for exactly that path) |
